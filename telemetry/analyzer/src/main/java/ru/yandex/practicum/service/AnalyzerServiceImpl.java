...
public class AnalyzerServiceImpl implements AnalyzerService {
    ...
    public void addDevice(DeviceAddedEventAvro deviceAdded, String hubId) {
        sensorRepository.save(new HomeSensor(deviceAdded.getId(), hubId));
    }
    ...
    public void addScenario(ScenarioAddedEventAvro scenarioAdded, String hubId) {
        ...
        if (!conditions.isEmpty()) {
            ...
            Map<String, HomeSensor> conditionSensors = sensorRepository.findAllByIdIn(conditionSensorIds) // Получаю все датчики
                    .stream()
                    .collect(Collectors.toMap(HomeSensor::getId, Function.identity()));
            ...
            for (int i = 0; i < conditions.size(); i++) { // Создаю связи между условиями, сенсорами и сценариями
                ...
                HomeSensor sensor = conditionSensors.get(conditionAvro.getSensorId());
        ...
        if (!actions.isEmpty()) {
            ...
            Map<String, HomeSensor> actionSensors = sensorRepository.findAllByIdIn(actionSensorIds) // Получаю все датчики
                    .stream()
                    .collect(Collectors.toMap(HomeSensor::getId, Function.identity()));
            ...
            for (int i = 0; i < actions.size(); i++) { // Создаю связи между действиями, сенсорами и сценариями
                ...
                HomeSensor sensor = actionSensors.get(actionAvro.getSensorId());
        ...
        for (Scenario scenario : scenarios) {
            ...
            if (isCompleted) {
                ...
                for (ScenarioAction scenarioAction : scenariosActions) {
                    HomeSensor sensor = scenarioAction.getSensor();
